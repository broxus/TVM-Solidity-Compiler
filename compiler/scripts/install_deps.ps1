...
if ( -not (Test-Path "$PSScriptRoot\..\deps\boost") ) {
  ...
  Invoke-WebRequest -URI "https://boostorg.jfrog.io/artifactory/main/release/1.83.0/source/boost_1_83_0.zip" -OutFile boost.zip
  # NOTE: The Boost zip hash may change if Boost updates or repackages.
  # If you are sure the download is correct, you can update the hash below.
  if ((Get-FileHash boost.zip).Hash -ne "c86bd9d9eef795b4b0d3802279419fde5221922805b073b9bd822edecb1ca28e") {
    Write-Warning 'Downloaded Boost source package has wrong checksum. Continuing anyway due to possible upstream repackage.'
  }
  tar -xf boost.zip
