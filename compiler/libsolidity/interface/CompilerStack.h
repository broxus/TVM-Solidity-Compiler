...
public:
	...
	bool didCompileSomething() const { return m_didCompileSomething; }

	void setMainContract(std::string const& mainContract) {
		// TODO DELETE USE selectContracts ?
	...
	bool doGenerateTvc() const {
		return m_generateCode && m_didCompileSomething;
	}
	...
	/// @returns false on error.
	bool compile(State _stopAfter = State::CompilationSuccessful, bool json = false);
	...
	virtual std::vector<std::string> mainContractName();
	...
	Json const& contractCode(std::string const& _contractName) const;
	Json const& functionIds(std::string const& _contractName) const;
	Json const& privateFunctionIds(std::string const& _contractName) const;
	...
	std::string getOutDirPathAndStem() const;
...
private:
	...
	struct Contract
	{
		...
		util::LazyInit<Json const> code;
		util::LazyInit<Json const> functionIds;
		util::LazyInit<Json const> privateFunctionIds;
	...
	Json const& contractCode(Contract const&) const;
	Json const& functionIds(Contract const&) const;
	Json const& privateFunctionIds(Contract const&) const;
	...
	bool m_didCompileSomething = false;
	std::string m_mainContract;
	bool m_generateAbi = false;
	bool m_generateCode = false;
	std::string m_folder;
