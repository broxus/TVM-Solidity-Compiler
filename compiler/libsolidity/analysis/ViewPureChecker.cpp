	...
	case Type::Category::Magic:
	{
		...
		if (pureMembers.empty()) {
			pureMembers.insert({
				...
				{MagicType::Kind::RIST255, "add"},
				{MagicType::Kind::RIST255, "fromHash"},
				{MagicType::Kind::RIST255, "l"},
				{MagicType::Kind::RIST255, "mul"},
				...
				{MagicType::Kind::RIST255, "validate"},
				...
				{MagicType::Kind::Transaction, "storageFees"},
				{MagicType::Kind::Transaction, "timestamp"},
			});
			for (auto const& op: tonCombinedArithmeticOperations()) {
				pureMembers.emplace(MagicType::Kind::Math, op.name);
			}
		}
