 ...
 * Support `bool` type as type for mapping's key.
 ...
 * If the contract does not have a constructor, then expression [new MyContract{value: ..., ...}();](API.md#deploy-via-new) generates a message
with empty body. Previously, it generated messages with calling default constructor ID (`0x68b55f3f`) - 
an empty constructor without parameters.
It is also true for [abi.encodeData()](API.md#abiencodedata) and [abi.encodeIntMsg()](API.md#abiencodeintmsg). 
* Print an error if the option "pubkey" is not set when deploying a contract that receives
  external messages or when using [abi.encodeStateInit()](API.md#abiencodestateinit)/[abi.encodeData()](./API.md#abiencodedata)
  for such a contract.
* Fixed a problem with [Function Calls with Named Parameters](https://docs.soliditylang.org/en/latest/control-structures.html#function-calls-with-named-parameters).
 ...
 * Keyword `nostorage` is deleted. Use keyword [transient](API.md#keyword-transient) instead of `nostorage`. 
 * Print an error if a public function takes only internal messages and returns some values.
 * Responsible functions can not take external messages.
 * A function can not receive external and internal messages simultaneously.
 * Removed keyword `internalMsg`
 * Removed `msg.isInternal`, `msg.isExternal`, `msg.isTickTock` and `msg.hasStateInit`.
 * `msg.createdAt`, `msg.sender` and `msg.currencies` can be used only in internal transactions.
 * Public key ([msg.pubkey()](API.md#msgpubkey), [tvm.pubkey()](API.md#tvmpubkey), 
[abi.encodeStateInit()](API.md#abiencodestateinit), etc.) has `bytes32` type instead of `uint256`.
 * `Bytes`, `string`, `TvmCell` types can not be used as a key type of `mapping`. E.g., instead of `mapping(string => ...)` use
`mapping(uint => ...)`. 
 ...
 * Generate more effective code for creating messages. More compact packing: inlining `stateInit` and body in the
   message body if it's possible.
 * Use `ihr_disabled=True` in messages generated by responsible functions and in messages built by [abi.encodeIntMsg](API.md#abiencodeintmsg).
 * Added cmd-line option `--debug` to disable optimizations.
 * Inline function call if the function is pretty small.
 * Replace `CALLREF { .inline function_name }` to `PUSHCONT { .inline function_name } CALLX` if `function_name` is called once.
 * Delete useless checks before calling `c4_to_c7` function.
 * Assorted stack optimizations.
 * Update compiler frontend (from original version 0.8.24 to 0.8.30). Full changelog can be found [here](./compiler/Changelog.md).
 * Added in `*.abi.json` files:
   * `input_id` - [input function ID](https://github.com/everx-labs/ever-abi/blob/master/src/function.rs#L66-L73)
   * `output_id` - output function ID
   * `externalMsg` - ability of the function to receive external messages
 ...
 * Fixed segmentation fault that occurred in some cases of using incorrect types for `math.*` and some other functions.
 ...
 * Supported [keyword nostorage](API.md#keyword-transient).
...
Compiler features:
 * Supported generating code for The Open Network. Use command line option `--tvm-version ton`. We don't use `ZERO(SWAP|ROTR)IF[NOT][2]`, `COPYLEFT`, `INITCODEHASH`, `MYCODE`, `STORAGEFEE`, `LDCONT`, `STCONT` and other opcodes for ton.

Optimizations:
 * Improved constant analyzer that compile more optimal code for `require`/`revert` functions and other cases.

Bugfixes:
 * Fixed segmentation fault that could occur in some cases of using `variable integer` types.
 * Fixed segmentation fault that occurred in constructions `optional(T1, T2) x = null;` and some other cases. 
...
Update compiler frontend (from original version 0.6.3 to 0.8.17):
  * Breaking changes:
    ...
    * And other breaking changes from original compiler.
  * Files are imported according [this](https://docs.soliditylang.org/en/v0.8.17/path-resolution.html). Use cmd options `--base-path`, `--include-path` and other options.
...
Compiler features:
 * Support mappings/arrays containing big structs (or other structs) as input/output parameters
...
Use other opcodes for working with `mapping(KeyType => TvmSlice) map;` So if you used it then be
...
* Support option `prefixLength` in `new` expression and in `tvm.buildStateInit(...)`.
 ...
 * Support api for getting info about size of DAG: `cell.dataSize`, `slice.dataSize` and others.
 * Support api functions for converting integer or address to a string: `format()`, `string(int)` and `hexstring`.
 * Support api for pseudo-random number generator: `rnd.next()`, `rnd.shuffle()`  and others.
